...
test.describe('Spec2CAD Project Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/projects/spec2cad');
  });

  test('page title includes project name', async ({ page }) => {
    await expect(page).toHaveTitle(/Spec2CAD.*Leonel Lourenco/);
  });
  ...
  test('H1 displays "Spec2CAD2"', async ({ page }) => {
    const h1 = page.locator('h1');
    await expect(h1).toHaveText('Spec2CAD2');
  });
