...
test.describe('Phantom Project Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/projects/phantom');
  });

  test('page title includes project name', async ({ page }) => {
    await expect(page).toHaveTitle(/Phantom.*Leonel Lourenco/);
  });
  ...
  test('H1 displays "Phantom"', async ({ page }) => {
    const h1 = page.locator('h1');
    await expect(h1).toHaveText('Phantom');
  });
  ...
  test('video demo info banner has correct text', async ({ page }) => {
    const banner = page.locator('text=Watch Phantom in action');
    await expect(banner).toBeVisible();
  });

  test('PhantomVideo React island mounts', async ({ page }) => {
